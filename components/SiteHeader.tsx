...
const NAV_ITEMS: Array<{ href: NavHref; label: string }> = [
  ...
  { href: { pathname: '/', hash: 'toy-projects' }, label: '토이' },
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-primary/10 bg-secondary/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden gap-4 text-sm font-medium text-slate md:flex">
          {NAV_ITEMS.map((item) => (
            <Link
              key={`nav-${item.href.hash ?? 'root'}`}
              href={item.href}
              className="rounded-full px-3 py-1 transition-colors duration-150 ease-out hover:bg-primary/10 hover:text-primary"
            >
              {item.label}
            </Link>
          ))}
